...
const { buildToolForgePlan } = require("./k7-tool-forge");
...
function buildSuperMetaskillRun(objective = "", options = {}) {
  ...
  const forge = buildToolForgePlan(goal, options);
  ...
  return {
    ...
    decision_pipeline: [
      ...
      {
        step: "commons",
        output: "free-first local/BYO route candidates before paid or hosted services",
      },
      {
        step: "trust",
      ...
      {
        step: "forge",
        output: "learn the external tool pattern and create the smallest agent-ready adapter",
      },
      {
        step: "execute",
    ...
    open_commons: {
      ...
      paid_default: commons.paid_services_policy.default,
    },
    production_control: {
    ...
    tool_strategy: {
      ...
      forge: {
        schema: forge.schema,
        lane: forge.selected_lane.id,
        adapter_shape: forge.selected_lane.adapter_shape,
        promotion_after: forge.promotion_gate.promote_after_verified_receipts,
      },
      anything_route: {
    ...
    execution_card: {
      ...
      context_command: `npm.cmd run k7 -- context "${safeGoal}"`,
      commons_command: `npm.cmd run k7 -- commons "${safeGoal}"`,
      forge_command: `npm.cmd run k7 -- forge "${safeGoal}"`,
...
function formatSuperMetaskillRun(run = buildSuperMetaskillRun()) {
  return [
    ...
    "",
    "## Open Commons",
    `- Doctrine: ${run.open_commons.doctrine}`,
    `- Top candidate: ${run.open_commons.top_candidate.id}`,
    `- Paid default: ${run.open_commons.paid_default}`,
    "",
    "## Production Control",
    ...
    `- Commons: ${run.execution_card.commons_command}`,
    ...
    `- Forge: ${run.execution_card.forge_command}`,
