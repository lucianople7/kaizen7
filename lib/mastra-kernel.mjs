...
function getBestAvailableModel() {
  // Mastra model IDs are "provider/model" (slash-delimited), not "provider:model".
  if (process.env.ARK_API_KEY) return "openai/ark-code-latest";
  if (process.env.DEEPSEEK_API_KEY) return "openai/deepseek-chat";
  if (process.env.QWEN_API_KEY) return "openai/qwen-max";
  if (process.env.ZAI_API_KEY) return "openai/zai-model";
  if (process.env.KIMI_API_KEY) return "openai/moonshot-v1-32k";
