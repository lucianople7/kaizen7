#!/usr/bin/env node
const {
  buildAnythingNextBlueprint,
  buildAnythingRoute,
  formatAnythingNextBlueprint,
  formatAnythingRoute,
} = require("./k7-anything-next");
const {
  appendMemoryEvent,
  buildAgentContext,
  formatAgentContext,
  formatMemoryAppend,
  parseMemoryInput,
} = require("./k7-agent-memory");
const { buildBestReport, formatAgentHandoff, formatBestReport } = require("./k7-best");
const { buildEvalPack, formatEvalPack } = require("./k7-eval-pack");
const { buildImprovementRadar, formatImprovementRadar } = require("./k7-improvement-radar");
const { buildMarketAdaptationPack, formatMarketAdaptationPack } = require("./k7-market-adaptation-pack");
const { buildMarketMap, formatMarketMap } = require("./k7-market-map");
const { buildMetaskillCard, formatMetaskillCard } = require("./k7-metaskill-card");
const { buildOpenCommonsPack, formatOpenCommonsPack } = require("./k7-open-commons");
const { buildProductionPack, formatProductionPack } = require("./k7-production-pack");
const { buildPreflight, formatPreflight } = require("./k7-preflight");
const { formatActionReactionLoop, runActionReactionLoop } = require("./k7-action-reaction-loop");
const { buildLoopSystemDefinition, formatLoopSystemDefinition } = require("./k7-loop-system");
const { formatOneDoor, runOneDoor } = require("./k7-one-door");
const {
  appendReceipt,
  buildReceiptRecall,
  formatReceiptAppend,
  formatReceiptRecall,
  parseReceiptInput,
} = require("./k7-receipt-ledger");
const { buildToolMeshPack, formatToolMeshPack } = require("./k7-tool-mesh-pack");
const { buildToolForgePlan, formatToolForgePlan } = require("./k7-tool-forge");
const { buildTrustGate, formatTrustGate } = require("./k7-trust-gate");
const { buildK7Status, formatK7Status } = require("./k7-status");
const { buildMissionControl, formatMissionControl } = require("./mission-control");
const { buildSelfImprovementLoop, formatSelfImprovementLoop } = require("./self-improvement-loop");
const { buildSmokeReport, formatSmokeReport } = require("./k7-smoke");
const { buildSuperMetaskillRun, formatSuperMetaskillRun } = require("./k7-super-metaskill");
const {
  buildWizardPlan,
  formatWizardPlan,
  parseWizardInput,
  runInteractiveWizard,
} = require("./k7-wizard");
const packageJson = require("../package.json");

const COMMANDS = [
  {
    name: "help",
    usage: "npm.cmd run k7 -- help",
    value: "Show the KAIZEN7 tool surface.",
  },
  {
    name: "status",
    usage: "npm.cmd run k7 -- status",
    aliases: ["s"],
    value: "Show branch, readiness, repo shape, risks and next mission.",
  },
  {
    name: "run",
    usage: "npm.cmd run k7 -- run \"<objective>\"",
    aliases: ["go"],
    value: "Return the KAIZEN7 super-metaskill run card: minimal context, radar, adaptation, tool route, verification and receipt.",
  },
  {
    name: "do",
    usage: "npm.cmd run k7 -- do \"<objective>\"",
    aliases: ["one", "one-door"],
    value: "The universal KAIZEN7 entry: decide, contract, route, verify, learn and return one standard envelope.",
  },
  {
    name: "preflight",
    usage: "npm.cmd run k7 -- preflight --budget 300 \"<objective>\"",
    aliases: ["pf"],
    value: "Reuse memory, decide research or approval, reject irrelevant candidates and return one compact Work/Codex route.",
  },
  {
    name: "loop",
    usage: "npm.cmd run k7 -- loop --max-iterations 8 \"<objective>\"",
    aliases: ["action-reaction"],
    value: "Turn a preflight into a bounded task, verification, receipt, learning gate and one next action.",
  },
  {
    name: "system",
    usage: "npm.cmd run k7 -- system",
    aliases: ["os", "loop-os"],
    value: "Inspect the canonical KAIZEN7 Loop OS roles, flow, autonomy, limits, gates and learning policy.",
  },
  {
    name: "wizard",
    usage: "npm.cmd run k7 -- wizard",
    aliases: ["start", "setup-guide"],
    value: "Guide a new user through objective, project type, free-first mode, connections, agent and next commands.",
  },
  {
    name: "doctor",
    usage: "npm.cmd run k7 -- doctor",
    aliases: ["d"],
    value: "Check the professional tool surface and readiness contract.",
  },
  {
    name: "version",
    usage: "npm.cmd run k7 -- version",
    aliases: ["v", "--version"],
    value: "Show package version, command count and schema.",
  },
  {
    name: "best",
    usage: "npm.cmd run k7 -- best",
    aliases: ["b"],
    value: "Show the best current operating view and agent handoff.",
  },
  {
    name: "handoff",
    usage: "npm.cmd run k7 -- handoff",
    aliases: ["h"],
    value: "Give another agent the smallest useful execution packet.",
  },
  {
    name: "mission",
    usage: "npm.cmd run k7 -- mission \"<objective>\"",
    aliases: ["m"],
    value: "Turn an objective into Growth Gate, route, Mission Brief and receipt template.",
  },
  {
    name: "solve",
    usage: "npm.cmd run k7 -- solve \"<objective>\"",
    aliases: ["key", "llave"],
    value: "Return the KAIZEN7 metaskill card: minimal memory, route, tool ladder, API escape, verification and learning.",
  },
  {
    name: "anything",
    usage: "npm.cmd run k7 -- anything \"<objective>\"",
    aliases: ["a"],
    value: "Build an agent-agnostic Anything CLI route, or show the blueprint with no objective.",
  },
  {
    name: "mesh",
    usage: "npm.cmd run k7 -- mesh \"<objective>\"",
    aliases: ["frontier", "steroids"],
    value: "Return the stronger KAIZEN7 Tool Mesh Pack: pillars, adapter pack, scoring, frontier modules and acceptance tests.",
  },
  {
    name: "adapt",
    usage: "npm.cmd run k7 -- adapt \"<objective>\"",
    aliases: ["market", "evolve"],
    value: "Return the modular market adaptation pack: open connection contracts, signal sources, evolution gates and retire/refresh rules.",
  },
  {
    name: "radar",
    usage: "npm.cmd run k7 -- radar \"<objective>\"",
    aliases: ["watch", "scan"],
    value: "Remind agents to search for current better routes, tools, repos and adapters before hardening a decision.",
  },
  {
    name: "opportunity",
    usage: "npm.cmd run k7 -- opportunity \"<objective>\"",
    aliases: ["problems", "diagnose", "value"],
    value: "Map constant market problems to KAIZEN7 routes, trust gates, budgets, verification and receipt memory.",
  },
  {
    name: "commons",
    usage: "npm.cmd run k7 -- commons \"<objective>\"",
    aliases: ["free", "oss", "libre"],
    value: "Return the free-first open commons pack: local-first patterns, BYO connections and no vendor lock-in.",
  },
  {
    name: "forge",
    usage: "npm.cmd run k7 -- forge \"<tool-need>\"",
    aliases: ["forja", "learn-tool", "pattern"],
    value: "Turn a tool need into radar, pattern learning, smallest adapter, verification, receipt memory and promotion rules.",
  },
  {
    name: "trust",
    usage: "npm.cmd run k7 -- trust \"<tool-or-connector>\"",
    aliases: ["gate", "security"],
    value: "Run the pre-execution trust gate for tools, MCPs, CLIs, browser agents and adapters.",
  },
  {
    name: "eval",
    usage: "npm.cmd run k7 -- eval \"<objective>\"",
    aliases: ["tests", "verify-plan"],
    value: "Generate the minimum eval pack: acceptance tests, regressions, budget, observability and receipt.",
  },
  {
    name: "production",
    usage: "npm.cmd run k7 -- production \"<objective>\"",
    aliases: ["prod", "ready"],
    value: "Return the production readiness pack: stack choice, trust gate, evals, observability and checklist.",
  },
  {
    name: "improve",
    usage: "npm.cmd run k7 -- improve \"<friction>\"",
    aliases: ["i"],
    value: "Run KAIZEN7 against KAIZEN7 and return a controlled self-improvement pass.",
  },
  {
    name: "recall",
    usage: "npm.cmd run k7 -- recall \"<objective>\"",
    aliases: ["memory"],
    value: "Search the local receipt ledger for reusable routes, tools and discard rules.",
  },
  {
    name: "context",
    usage: "npm.cmd run k7 -- context \"<objective>\"",
    aliases: ["ctx"],
    value: "Generate a local, agent-agnostic context pack with trust boundary, receipts and next actions.",
  },
  {
    name: "resume",
    usage: "npm.cmd run k7 -- resume \"<objective>\"",
    aliases: ["continue"],
    value: "Resume an agent or project from local KAIZEN7 context without replaying broad history.",
  },
  {
    name: "journal",
    usage: "npm.cmd run k7 -- journal \"<event-json>\"",
    aliases: ["log"],
    value: "Append an agent-agnostic local memory event for future context and resume packs.",
  },
  {
    name: "remember",
    usage: "npm.cmd run k7 -- remember \"<receipt-json>\"",
    aliases: ["learn"],
    value: "Append a verified Mission Outcome Receipt to the local KAIZEN7 receipt ledger.",
  },
  {
    name: "receipt",
    usage: "npm.cmd run k7 -- receipt",
    aliases: ["r"],
    value: "Show the closeout fields every mission should return.",
  },
  {
    name: "check",
    usage: "npm.cmd run k7 -- check",
    aliases: ["c"],
    value: "Run the compact KAIZEN7 smoke report. Use npm.cmd run k7:check for the full suite.",
  },
];

function buildK7Tool() {
  return {
    schema: "kaizen7.tool.v1",
    name: "k7",
    identity: "KAIZEN7 official agent-agnostic project improvement tool",
    promise: "Turn any project objective into minimal context, an Anything CLI route, verified execution and reusable learning.",
    principle: "Human decides. KAIZEN7 coordinates. Agents execute. Projects grow.",
    flow: [
      "objective",
      "metaskill",
      "minimal memory",
      "route",
      "Anything CLI discovery",
      "skill/metaskill",
      "executor",
      "verification",
      "receipt",
      "memory recommendation",
    ],
    commands: COMMANDS,
  };
}

function formatK7ToolHelp(tool = buildK7Tool()) {
  return [
    "# KAIZEN7 TOOL",
    "",
    tool.promise,
    "",
    tool.principle,
    "",
    "## Flow",
    tool.flow.join(" -> "),
    "",
    "## Commands",
    ...tool.commands.map((command) => {
      const aliases = command.aliases?.length ? ` [${command.aliases.join(", ")}]` : "";
      return `- ${command.usage}${aliases}: ${command.value}`;
    }),
    "",
    "## Start Here",
    "- npm.cmd run k7 -- do \"<objective>\"",
    "- npm.cmd run k7 -- status",
    "- npm.cmd run k7 -- wizard",
    "- npm.cmd run k7 -- run \"<objective>\"",
    "- npm.cmd run k7 -- solve \"<objective>\"",
    "- npm.cmd run k7 -- mission \"<objective>\"",
    "- npm.cmd run k7 -- mesh \"<objective>\"",
    "- npm.cmd run k7 -- adapt \"<objective>\"",
    "- npm.cmd run k7 -- radar \"<objective>\"",
    "- npm.cmd run k7 -- improve \"<friction>\"",
    "- npm.cmd run k7 -- handoff",
    "",
  ].join("\n");
}

function parseCommand(argv = []) {
  const [command = "help", ...rest] = argv;
  return {
    command: resolveCommandName(command.toLowerCase()),
    args: rest,
    wantsJson: rest.includes("--json"),
  };
}

function resolveCommandName(input = "help") {
  const normalized = String(input || "help").toLowerCase();
  if (normalized === "-h" || normalized === "--help") return "help";
  const match = COMMANDS.find((command) => (
    command.name === normalized || (command.aliases || []).includes(normalized)
  ));
  return match?.name || normalized;
}

function suggestCommandName(input = "") {
  const normalized = String(input || "").toLowerCase();
  if (!normalized) return "help";
  const names = COMMANDS.flatMap((command) => [command.name, ...(command.aliases || [])]);
  return names
    .map((name) => ({
      name,
      score: commonPrefixLength(normalized, name),
    }))
    .sort((left, right) => right.score - left.score || left.name.localeCompare(right.name))[0]?.name || "help";
}

function commonPrefixLength(left, right) {
  let count = 0;
  while (count < left.length && count < right.length && left[count] === right[count]) count += 1;
  return count;
}

function commandText(args = []) {
  return args.filter((arg) => !arg.startsWith("--")).join(" ").trim();
}

function flagValue(args = [], name = "") {
  const index = args.indexOf(name);
  return index >= 0 ? args[index + 1] : undefined;
}

function commandTextWithoutValueFlags(args = [], flags = []) {
  const skipped = new Set();
  for (const flag of flags) {
    const index = args.indexOf(flag);
    if (index >= 0) {
      skipped.add(index);
      skipped.add(index + 1);
    }
  }
  return args.filter((arg, index) => !skipped.has(index) && !arg.startsWith("--")).join(" ").trim();
}

function buildVersionPacket(tool = buildK7Tool()) {
  return {
    schema: "kaizen7.version.v1",
    name: packageJson.name,
    version: packageJson.version,
    tool: tool.name,
    command_count: tool.commands.length,
    commands: tool.commands.map((command) => command.name),
    node: process.version,
  };
}

function formatVersionPacket(packet = buildVersionPacket()) {
  return [
    "# KAIZEN7 VERSION",
    "",
    `Package: ${packet.name}`,
    `Version: ${packet.version}`,
    `Tool: ${packet.tool}`,
    `Commands: ${packet.command_count}`,
    `Node: ${packet.node}`,
    "",
  ].join("\n");
}

function buildDoctorReport(root = process.cwd()) {
  const tool = buildK7Tool();
  const status = buildK7Status(root);
  const requiredCommands = ["status", "run", "do", "preflight", "loop", "system", "wizard", "mission", "solve", "handoff", "anything", "mesh", "adapt", "radar", "opportunity", "commons", "forge", "trust", "eval", "production", "improve", "recall", "context", "resume", "journal", "remember", "receipt", "check"];
  const checks = [
    {
      ok: tool.schema === "kaizen7.tool.v1",
      label: "tool surface schema",
    },
    {
      ok: requiredCommands.every((name) => tool.commands.some((command) => command.name === name)),
      label: "tool surface commands",
    },
    {
      ok: status.readiness.status === "ready",
      label: "readiness",
      detail: status.readiness.status,
    },
    {
      ok: status.next_mission.route === "mission_brief",
      label: "route language",
      detail: status.next_mission.route,
    },
    {
      ok: buildAnythingRoute("improve this project").schema === "kaizen7.anything_route.v1",
      label: "anything route contract",
    },
    {
      ok: buildMetaskillCard("connect to an app without an API").schema === "kaizen7.metaskill_card.v1",
      label: "metaskill card contract",
    },
    {
      ok: buildToolMeshPack("connect to apps without APIs").schema === "kaizen7.tool_mesh_pack.v1",
      label: "tool mesh pack contract",
    },
    {
      ok: buildMarketAdaptationPack("keep connectors modular").schema === "kaizen7.market_adaptation_pack.v1",
      label: "market adaptation contract",
    },
    {
      ok: buildImprovementRadar("find a better current agent browser").schema === "kaizen7.improvement_radar.v1",
      label: "improvement radar contract",
    },
    {
      ok: buildMarketMap("agentic market problems", { root }).schema === "kaizen7.market_map.v1",
      label: "market map contract",
    },
    {
      ok: buildOpenCommonsPack("free agent routes").schema === "kaizen7.open_commons_pack.v1",
      label: "open commons contract",
    },
    {
      ok: buildToolForgePlan("learn a video tool").schema === "kaizen7.tool_forge_plan.v1",
      label: "tool forge contract",
    },
    {
      ok: buildTrustGate("MCP browser tool").schema === "kaizen7.trust_gate.v1",
      label: "trust gate contract",
    },
    {
      ok: buildEvalPack("agentic mission").schema === "kaizen7.eval_pack.v1",
      label: "eval pack contract",
    },
    {
      ok: buildProductionPack("agentic production workflow").schema === "kaizen7.production_pack.v1",
      label: "production pack contract",
    },
    {
      ok: buildSuperMetaskillRun("improve an agent").schema === "kaizen7.super_metaskill_run.v1",
      label: "super metaskill run contract",
    },
    {
      ok: buildWizardPlan({ objective: "improve a repo" }).schema === "kaizen7.wizard_plan.v1",
      label: "wizard contract",
    },
    {
      ok: buildReceiptRecall("improve an agent", { root }).schema === "kaizen7.receipt_recall.v1",
      label: "receipt ledger recall contract",
    },
    {
      ok: buildPreflight("stable code test", { root }).schema === "kaizen7.preflight_card.v1",
      label: "Work preflight contract",
    },
    {
      ok: runActionReactionLoop("stable code test", { root }).schema === "kaizen7.action_reaction_loop.v1",
      label: "Action reaction loop contract",
    },
    {
      ok: buildLoopSystemDefinition().schema === "kaizen7.loop_system.v1",
      label: "Canonical loop system policy",
    },
    {
      ok: runOneDoor("stable code test", { root }).schema === "kaizen7.one_door.v1",
      label: "One Door universal envelope",
    },
    {
      ok: buildAgentContext("resume an agent", { root, write: false }).schema === "kaizen7.agent_context_pack.v1",
      label: "agent context contract",
    },
    {
      ok: packageJson.bin?.k7 === "lib/k7-cli.js",
      label: "k7 binary entrypoint",
      detail: packageJson.bin?.k7,
    },
  ];
  return {
    schema: "kaizen7.doctor.v1",
    status: checks.every((check) => check.ok) ? "pass" : "fail",
    checks,
    next_command: "npm.cmd run k7 -- mission \"<objective>\"",
  };
}

function formatDoctorReport(report = buildDoctorReport()) {
  return [
    "# KAIZEN7 DOCTOR",
    "",
    `Status: ${report.status}`,
    "",
    "## Checks",
    ...report.checks.map((check) => `- ${check.ok ? "PASS" : "FAIL"} ${check.label}${check.detail ? ` (${check.detail})` : ""}`),
    "",
    "## Next",
    `- ${report.next_command}`,
    "",
  ].join("\n");
}

function buildReceiptTemplate() {
  return {
    schema: "kaizen7.receipt_template.v1",
    title: "Mission Outcome Receipt",
    fields: {
      summary: "",
      changed_files: [],
      tests: [],
      risks: [],
      reuse_next_time: "",
      discard: [],
      memory_update_recommendation: "",
      next_action: "",
    },
  };
}

function buildSelfImprovementPass(loop) {
  const goal = loop.goal || "mejorar KAIZEN7";
  return {
    schema: "kaizen7.self_improvement_pass.v1",
    mode: "controlled_self_improvement",
    status: loop.status,
    subject: loop.subject,
    goal,
    friction: `Improve KAIZEN7 against this friction: ${goal}`,
    decision: loop.verdict,
    improvement_rule: "Patch only one kernel friction at a time, prove it with a test, then run the check suite.",
    selected_metaskills: [
      "k7-self-evolution-loop",
      "kaizen7-evolution-engine",
      "test-driven-development",
      "verification-before-completion",
    ],
    actions: [
      "Identify one concrete friction from current command output.",
      "Write or update the smallest failing test for that friction.",
      "Patch only the module that owns the behavior.",
      "Run focused tests, then npm.cmd run k7:check.",
      "Close with a Mission Outcome Receipt and memory recommendation.",
    ],
    commands: [
      "npm.cmd run k7 -- improve \"<friction>\"",
      "node tests/k7-cli.test.js",
      "npm.cmd run k7 -- check",
      "npm.cmd run k7:check",
    ],
    gates: loop.gates,
    next_action: "Pick the smallest friction in the k7 command surface and convert it into a tested patch.",
    receipt: {
      summary: "",
      changed_files: [],
      tests: [],
      risks: loop.gates,
      reuse_next_time: "Start with k7 improve for tool-surface friction before broad refactors.",
      discard: ["broad autonomous rewrites", "unverified improvement claims", "memory noise"],
      memory_update_recommendation: "Record only if the pass reduces future steps, context or errors.",
    },
  };
}

function formatSelfImprovementPass(pass) {
  return [
    "# KAIZEN7 SELF IMPROVEMENT PASS",
    "",
    `Status: ${pass.status}`,
    `Goal: ${pass.goal}`,
    `Decision: ${pass.decision}`,
    "",
    "## Friction",
    `- ${pass.friction}`,
    "",
    "## Rule",
    `- ${pass.improvement_rule}`,
    "",
    "## Metaskills",
    ...pass.selected_metaskills.map((skill) => `- ${skill}`),
    "",
    "## Actions",
    ...pass.actions.map((action) => `- ${action}`),
    "",
    "## Commands",
    ...pass.commands.map((command) => `- ${command}`),
    "",
    "## Gates",
    ...pass.gates.map((gate) => `- ${gate}`),
    "",
    "## Receipt",
    ...Object.keys(pass.receipt).map((key) => `${key}:`),
    "",
  ].join("\n");
}

function formatReceiptTemplate(receipt = buildReceiptTemplate()) {
  return [
    "# Mission Outcome Receipt",
    "",
    ...Object.keys(receipt.fields).map((field) => `${field}:`),
    "",
  ].join("\n");
}

function runK7ToolCommand(argv = [], root = process.cwd()) {
  const parsed = parseCommand(argv);
  const tool = buildK7Tool();

  if (parsed.command === "help" || parsed.command === "--help" || parsed.command === "-h") {
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(tool, null, 2)}\n` : `${formatK7ToolHelp(tool)}\n`,
    };
  }

  if (parsed.command === "status") {
    const status = buildK7Status(root);
    return {
      exitCode: status.readiness.status === "ready" ? 0 : 1,
      output: parsed.wantsJson ? `${JSON.stringify(status, null, 2)}\n` : `${formatK7Status(status)}\n`,
    };
  }

  if (parsed.command === "doctor") {
    const report = buildDoctorReport(root);
    return {
      exitCode: report.status === "pass" ? 0 : 1,
      output: parsed.wantsJson ? `${JSON.stringify(report, null, 2)}\n` : `${formatDoctorReport(report)}\n`,
    };
  }

  if (parsed.command === "version") {
    const packet = buildVersionPacket(tool);
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(packet, null, 2)}\n` : `${formatVersionPacket(packet)}\n`,
    };
  }

  if (parsed.command === "best") {
    const report = buildBestReport(root);
    return {
      exitCode: report.status === "pass" ? 0 : 1,
      output: parsed.wantsJson ? `${JSON.stringify(report, null, 2)}\n` : `${formatBestReport(report)}\n`,
    };
  }

  if (parsed.command === "handoff") {
    const report = buildBestReport(root);
    return {
      exitCode: report.status === "pass" ? 0 : 1,
      output: parsed.wantsJson ? `${JSON.stringify(report.agent_handoff, null, 2)}\n` : `${formatAgentHandoff(report.agent_handoff)}\n`,
    };
  }

  if (parsed.command === "mission") {
    const goal = commandText(parsed.args);
    const control = buildMissionControl({ goal: goal || "prepare KAIZEN7 mission" });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(control, null, 2)}\n` : `${formatMissionControl(control)}\n`,
    };
  }

  if (parsed.command === "run") {
    const goal = commandText(parsed.args);
    const run = buildSuperMetaskillRun(goal || "improve this project with less context and better verification", { json: parsed.wantsJson });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(run, null, 2)}\n` : `${formatSuperMetaskillRun(run)}\n`,
    };
  }

  if (parsed.command === "do") {
    try {
      const inputValue = flagValue(parsed.args, "--input");
      let input;
      if (inputValue !== undefined) {
        input = JSON.parse(inputValue);
      } else {
        const iterationValue = flagValue(parsed.args, "--iterations");
        const tokenValue = flagValue(parsed.args, "--tokens");
        input = {
          objective: commandTextWithoutValueFlags(parsed.args, ["--project", "--iterations", "--tokens"]),
          project: flagValue(parsed.args, "--project") || "KAIZEN7",
          budget: {
            iterations: iterationValue === undefined ? 8 : Number(iterationValue),
            tokens: tokenValue === undefined ? 1200 : Number(tokenValue),
          },
        };
      }
      const envelope = runOneDoor(input, { root });
      return {
        exitCode: 0,
        output: parsed.wantsJson ? `${JSON.stringify(envelope, null, 2)}\n` : formatOneDoor(envelope),
      };
    } catch (error) {
      return { exitCode: 2, output: `Invalid One Door input: ${error.message}\n` };
    }
  }

  if (parsed.command === "preflight") {
    try {
      const budgetValue = flagValue(parsed.args, "--budget");
      const budget = budgetValue === undefined ? 300 : Number(budgetValue);
      if (!Number.isInteger(budget)) throw new Error("budget must be an integer between 120 and 600");
      const goal = commandTextWithoutValueFlags(parsed.args, ["--budget", "--candidate"]);
      const card = buildPreflight(goal, {
        root,
        budget,
        candidate: flagValue(parsed.args, "--candidate") || "",
      });
      return {
        exitCode: 0,
        output: parsed.wantsJson ? `${JSON.stringify(card, null, 2)}\n` : formatPreflight(card, { budget }),
      };
    } catch (error) {
      return {
        exitCode: 2,
        output: `Invalid preflight: ${error.message}\n`,
      };
    }
  }

  if (parsed.command === "loop") {
    try {
      const maxValue = flagValue(parsed.args, "--max-iterations");
      const tokenValue = flagValue(parsed.args, "--token-budget");
      const maxIterations = maxValue === undefined ? 8 : Number(maxValue);
      const tokenBudget = tokenValue === undefined ? 1200 : Number(tokenValue);
      if (!Number.isInteger(maxIterations) || maxIterations < 1 || maxIterations > 20) {
        throw new Error("max-iterations must be an integer between 1 and 20");
      }
      if (!Number.isInteger(tokenBudget) || tokenBudget < 120 || tokenBudget > 10000) {
        throw new Error("token-budget must be an integer between 120 and 10000");
      }
      const goal = commandTextWithoutValueFlags(parsed.args, ["--max-iterations", "--token-budget"]);
      const loop = runActionReactionLoop(goal, { root, maxIterations, tokenBudget });
      return {
        exitCode: 0,
        output: parsed.wantsJson ? `${JSON.stringify(loop, null, 2)}\n` : formatActionReactionLoop(loop),
      };
    } catch (error) {
      return { exitCode: 2, output: `Invalid loop: ${error.message}\n` };
    }
  }

  if (parsed.command === "system") {
    const system = buildLoopSystemDefinition();
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(system, null, 2)}\n` : formatLoopSystemDefinition(system),
    };
  }

  if (parsed.command === "wizard") {
    try {
      const plan = buildWizardPlan(parseWizardInput(parsed.args));
      return {
        exitCode: 0,
        output: parsed.wantsJson ? `${JSON.stringify(plan, null, 2)}\n` : `${formatWizardPlan(plan)}\n`,
      };
    } catch (error) {
      return {
        exitCode: 2,
        output: [
          `Invalid wizard input: ${error.message}`,
          "Usage: npm.cmd run k7 -- wizard --answers \"<json>\"",
          "",
        ].join("\n"),
      };
    }
  }

  if (parsed.command === "solve") {
    const goal = commandText(parsed.args);
    const card = buildMetaskillCard(goal || "improve this project with fewer steps", { json: parsed.wantsJson });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(card, null, 2)}\n` : `${formatMetaskillCard(card)}\n`,
    };
  }

  if (parsed.command === "anything") {
    const goal = commandText(parsed.args);
    const output = goal
      ? buildAnythingRoute(goal, { json: parsed.wantsJson })
      : buildAnythingNextBlueprint();
    return {
      exitCode: 0,
      output: parsed.wantsJson
        ? `${JSON.stringify(output, null, 2)}\n`
        : `${goal ? formatAnythingRoute(output) : formatAnythingNextBlueprint(output)}\n`,
    };
  }

  if (parsed.command === "mesh") {
    const goal = commandText(parsed.args);
    const pack = buildToolMeshPack(goal || "make KAIZEN7 solve harder external tool problems", { json: parsed.wantsJson });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(pack, null, 2)}\n` : `${formatToolMeshPack(pack)}\n`,
    };
  }

  if (parsed.command === "adapt") {
    const goal = commandText(parsed.args);
    const pack = buildMarketAdaptationPack(goal || "keep KAIZEN7 useful as tools and markets change", { json: parsed.wantsJson });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(pack, null, 2)}\n` : `${formatMarketAdaptationPack(pack)}\n`,
    };
  }

  if (parsed.command === "radar") {
    const goal = commandText(parsed.args);
    const radar = buildImprovementRadar(goal || "find a better current route before hardening the next decision", { json: parsed.wantsJson });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(radar, null, 2)}\n` : `${formatImprovementRadar(radar)}\n`,
    };
  }

  if (parsed.command === "opportunity") {
    const goal = commandText(parsed.args);
    const map = buildMarketMap(goal || "make KAIZEN7 valuable for real agentic project improvement", { root });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(map, null, 2)}\n` : `${formatMarketMap(map)}\n`,
    };
  }

  if (parsed.command === "commons") {
    const goal = commandText(parsed.args);
    const pack = buildOpenCommonsPack(goal || "keep KAIZEN7 free-first, open and agent-agnostic");
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(pack, null, 2)}\n` : `${formatOpenCommonsPack(pack)}\n`,
    };
  }

  if (parsed.command === "forge") {
    const goal = commandText(parsed.args);
    const plan = buildToolForgePlan(goal || "learn and forge a reusable agent-ready tool route");
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(plan, null, 2)}\n` : `${formatToolForgePlan(plan)}\n`,
    };
  }

  if (parsed.command === "trust") {
    const goal = commandText(parsed.args);
    const gate = buildTrustGate(goal || "unknown tool or connector");
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(gate, null, 2)}\n` : `${formatTrustGate(gate)}\n`,
    };
  }

  if (parsed.command === "eval") {
    const goal = commandText(parsed.args);
    const pack = buildEvalPack(goal || "verify an agentic mission before production");
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(pack, null, 2)}\n` : `${formatEvalPack(pack)}\n`,
    };
  }

  if (parsed.command === "production") {
    const goal = commandText(parsed.args);
    const pack = buildProductionPack(goal || "prepare an agentic workflow for production");
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(pack, null, 2)}\n` : `${formatProductionPack(pack)}\n`,
    };
  }

  if (parsed.command === "improve") {
    const goal = commandText(parsed.args);
    const loop = buildSelfImprovementLoop({
      root,
      goal: goal || "mejorar KAIZEN7 usando KAIZEN7",
    });
    if (parsed.args.includes("--raw")) {
      return {
        exitCode: loop.status === "blocked" ? 1 : 0,
        output: parsed.wantsJson ? `${JSON.stringify(loop, null, 2)}\n` : `${formatSelfImprovementLoop(loop)}\n`,
      };
    }
    const pass = buildSelfImprovementPass(loop);
    return {
      exitCode: loop.status === "blocked" ? 1 : 0,
      output: parsed.wantsJson ? `${JSON.stringify(pass, null, 2)}\n` : `${formatSelfImprovementPass(pass)}\n`,
    };
  }

  if (parsed.command === "recall") {
    const goal = commandText(parsed.args);
    const recall = buildReceiptRecall(goal || "improve this project with fewer steps", { root });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(recall, null, 2)}\n` : `${formatReceiptRecall(recall)}\n`,
    };
  }

  if (parsed.command === "context" || parsed.command === "resume") {
    const goal = commandText(parsed.args);
    const context = buildAgentContext(goal || "resume this project with minimal context", { root, write: parsed.command === "context" });
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(context, null, 2)}\n` : `${formatAgentContext(context)}\n`,
    };
  }

  if (parsed.command === "journal") {
    try {
      const eventInput = parseMemoryInput(commandText(parsed.args));
      const packet = appendMemoryEvent(eventInput, { root });
      return {
        exitCode: 0,
        output: parsed.wantsJson ? `${JSON.stringify(packet, null, 2)}\n` : `${formatMemoryAppend(packet)}\n`,
      };
    } catch (error) {
      return {
        exitCode: 2,
        output: [
          `Invalid memory event: ${error.message}`,
          "Usage: npm.cmd run k7 -- journal \"<event-json>\"",
          "",
        ].join("\n"),
      };
    }
  }

  if (parsed.command === "remember") {
    try {
      const receiptInput = parseReceiptInput(commandText(parsed.args));
      const packet = appendReceipt(receiptInput, { root });
      return {
        exitCode: 0,
        output: parsed.wantsJson ? `${JSON.stringify(packet, null, 2)}\n` : `${formatReceiptAppend(packet)}\n`,
      };
    } catch (error) {
      return {
        exitCode: 2,
        output: [
          `Invalid receipt: ${error.message}`,
          "Usage: npm.cmd run k7 -- remember \"<receipt-json>\"",
          "",
        ].join("\n"),
      };
    }
  }

  if (parsed.command === "receipt") {
    const receipt = buildReceiptTemplate();
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(receipt, null, 2)}\n` : `${formatReceiptTemplate(receipt)}\n`,
    };
  }

  if (parsed.command === "check") {
    const report = buildSmokeReport(root);
    return {
      exitCode: report.status === "pass" ? 0 : 1,
      output: parsed.wantsJson ? `${JSON.stringify(report, null, 2)}\n` : `${formatSmokeReport(report)}\n`,
    };
  }

  return {
    exitCode: 2,
    output: [
      `Unknown command: ${parsed.command}`,
      `Did you mean: ${suggestCommandName(parsed.command)}`,
      "",
      formatK7ToolHelp(tool),
    ].join("\n"),
  };
}

if (require.main === module) {
  main(process.argv.slice(2)).catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
}

async function main(argv = []) {
  const parsed = parseCommand(argv);
  const hasWizardInput = parsed.args.length > 0 || parsed.wantsJson;
  if (parsed.command === "wizard" && !hasWizardInput && process.stdin.isTTY) {
    await runInteractiveWizard();
    return;
  }
  const result = runK7ToolCommand(argv, process.cwd());
  process.stdout.write(result.output);
  process.exitCode = result.exitCode;
}

module.exports = {
  appendMemoryEvent,
  appendReceipt,
  buildAgentContext,
  buildDoctorReport,
  buildEvalPack,
  buildSelfImprovementPass,
  buildK7Tool,
  buildImprovementRadar,
  buildMarketAdaptationPack,
  buildMarketMap,
  buildMetaskillCard,
  buildOpenCommonsPack,
  buildProductionPack,
  buildPreflight,
  buildToolMeshPack,
  buildToolForgePlan,
  buildReceiptRecall,
  buildTrustGate,
  buildWizardPlan,
  buildReceiptTemplate,
  buildSuperMetaskillRun,
  buildVersionPacket,
  formatK7ToolHelp,
  formatImprovementRadar,
  formatMarketAdaptationPack,
  formatMarketMap,
  formatAgentContext,
  formatEvalPack,
  formatMemoryAppend,
  formatMetaskillCard,
  formatOpenCommonsPack,
  formatProductionPack,
  formatPreflight,
  formatToolMeshPack,
  formatToolForgePlan,
  formatDoctorReport,
  formatReceiptAppend,
  formatReceiptRecall,
  formatTrustGate,
  formatWizardPlan,
  formatReceiptTemplate,
  formatSelfImprovementPass,
  formatSuperMetaskillRun,
  formatVersionPacket,
  commandText,
  commandTextWithoutValueFlags,
  flagValue,
  resolveCommandName,
  suggestCommandName,
  parseCommand,
  parseWizardInput,
  runK7ToolCommand,
};
