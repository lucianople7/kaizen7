...
function buildFirstRun(options) {
  ...
  return {
    ...
    objective,
    commands: unique([
      `npm.cmd run k7:eval -- --project ${JSON.stringify(project)} --context ${JSON.stringify(context)} ${JSON.stringify(objective)}`,
      `npm.cmd run k7:cockpit -- --context ${JSON.stringify(context)} ${JSON.stringify(objective)}`,
    ]),
    deliverables: [
...
function buildEvalHarness(options = {}) {
  ...
  return {
    ...
    metrics,
    firstRun: buildFirstRun({ project, objective, context }),
    evidenceRequired: [
      "commands_run",
      "files_changed",
      "tests_or_checks",
