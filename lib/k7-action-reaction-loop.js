...
function executionTokenUsage(value) {
...
function ownerFor(preflight = {}, policy = loadLoopPolicy()) {
  ...
  if (["research_primary_sources", "reuse_receipt"].includes(preflight.route)) {
    return policy.routing[preflight.route];
  }
...
function runActionReactionLoop(objective = "", options = {}) {
  ...
  const maxIterations = positiveIntegerOption(options.maxIterations, policy.limits.max_iterations, "maxIterations");
  const maxFailures = positiveIntegerOption(options.maxFailures, policy.limits.max_failures, "maxFailures");
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration += 1) {
    ...
    const attemptTokenUsage = executionTokenUsage(execution.token_usage);
    if (attemptTokenUsage === null) {
      const invalidUsageVerification = {
        passed: false,
        correction: "Executor token_usage must be a non-negative integer.",
      };
      attempts.push({
        iteration,
        status: "failed",
        ...
        verification: invalidUsageVerification,
      });
      states.push("blocked");
      return terminal(base, "blocked", "Repair the executor token accounting before resuming.", {
        stop_reason: "invalid_token_usage",
        verification: invalidUsageVerification,
        tokens_used: tokensUsed,
      });
    }
    tokensUsed += attemptTokenUsage;
