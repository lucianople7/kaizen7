const { buildAnythingNextBlueprint, formatAnythingNextBlueprint } = require("./k7-anything-next");
...
const COMMANDS = [
  ...
  {
    name: "anything",
    usage: "npm.cmd run k7 -- anything",
    aliases: ["a"],
    value: "Show the next-generation Anything CLI tool mesh contract.",
  },
  {
    name: "improve",
...
function buildK7Tool() {
  return {
    ...
    name: "k7",
    identity: "KAIZEN7 high-value command surface",
    promise: "Turn a broad objective into less context, a better route, a safer executor and reusable learning.",
...
function buildDoctorReport(root = process.cwd()) {
  ...
  const requiredCommands = ["status", "mission", "handoff", "anything", "improve", "receipt", "check"];
  const checks = [
    {
...
function runK7ToolCommand(argv = [], root = process.cwd()) {
  ...
  if (parsed.command === "anything") {
    ...
    return {
      exitCode: 0,
      output: parsed.wantsJson ? `${JSON.stringify(blueprint, null, 2)}\n` : `${formatAnythingNextBlueprint(blueprint)}\n`,
    };
  }
