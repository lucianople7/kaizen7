...
function slugify(value = "forge-session") {
  return String(value || "forge-session")
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 80) || "forge-session";
}
...
function inferVerifyCommand(provider = {}) {
  if (!provider) return [];
  if (provider.id === "whisper") return ["whisper", "--help"];
  if (provider.id === "videocaptioner") return ["videocaptioner", "--help"];
  if (provider.id === "ffmpeg") return ["ffmpeg", "-version"];
  if (provider.id === "ollama") return ["ollama", "--version"];
  if (provider.kind === "node" && provider.commands?.plan) return String(provider.commands.plan).split(/\s+/);
  if (provider.kind === "binary") return [provider.id, "--help"];
  return [];
}

function buildAdapterManifestPlan(providerRadar, capability) {
  if (providerRadar.decision !== "adapt_provider" || !providerRadar.selected) return null;
  const provider = providerRadar.selected;
  return {
    schema: "kaizen7.adapter_manifest_plan.v1",
    version: 1,
    ...
    verify_command: inferVerifyCommand(provider),
    approval_required: ["install_binary", "download_model", "start_persistent_service"],
    evidence_expected: provider.evidence || [],
    output_files: [
      "data/anything-tools.json",
      `lib/adapters/${provider.id}-adapter.js`,
      `tests/${provider.id}-adapter.test.js`,
    ],
    next_action: "create_manifest_then_verify_provider",
  };
}

function inferAgentFirstMove(providerRadar) {
  if (providerRadar.decision === "use_provider") return "create_execution_packet";
  if (providerRadar.decision === "adapt_provider") return "create_or_update_provider_manifest";
  return "search_and_absorb_pattern";
}

function buildAgentBrief(packet) {
  const radar = packet.provider_radar || {};
  const adapter = packet.adapter_manifest;
  const approvals = Array.from(new Set([
    ...(packet.approval_required || []),
    ...((adapter && adapter.approval_required) || []),
  ]));
  const evidence = Array.from(new Set([
    ...(packet.evidence_required || []),
    ...((adapter && adapter.evidence_expected) || []),
  ]));
  return [
    ...
    `Provider decision: \`${radar.decision || "unknown"}\``,
    `Selected provider: \`${radar.selected?.id || "none"}\``,
    `Next action: \`${adapter?.next_action || packet.next_action}\``,
    "",
    "## First Move",
    "",
    packet.agent_packet?.first_move || "resolve_provider_or_absorb_pattern",
    "",
    "## Approval Required",
    "",
    ...
    "## Avoid",
    "",
    ...(packet.agent_packet?.avoid || []).map((item) => `- ${item}`),
    "",
  ].join("\n");
}

function normalizeAgentProfile(agent = "generic") {
  const value = String(agent || "generic").trim().toLowerCase();
  if (["codex", "claude-code", "openhands", "generic"].includes(value)) return value;
  if (value === "claude" || value === "cloud-code") return "claude-code";
  return "generic";
}

function buildAgentHandoff(packet, options = {}) {
  const targetAgent = normalizeAgentProfile(options.agent);
  const radar = packet.provider_radar || {};
  const adapter = packet.adapter_manifest;
  const files = adapter?.output_files || [];
  return [
    ...
    "## Files To Touch",
    "",
    ...(files.length ? files.map((file) => `- ${file}`) : ["- No file writes until a manifest or packet defines them."]),
    "",
    "## Hard Stops",
    "",
...
function writeForgeSession(packet, options = {}) {
  const root = options.root || ROOT;
  const sessionId = `${slugify(packet.objective)}-${packet.capability.replaceAll(".", "-")}`;
  const sessionDir = path.join(root, "content_library", "forge", "sessions", sessionId);
  const packetFile = path.join(sessionDir, "forge-packet.json");
  const briefFile = path.join(sessionDir, "agent-brief.md");
  const handoffFile = path.join(sessionDir, "agent-handoff.md");
  fs.mkdirSync(sessionDir, { recursive: true });
  fs.writeFileSync(packetFile, `${JSON.stringify(packet, null, 2)}\n`, "utf8");
  fs.writeFileSync(briefFile, buildAgentBrief(packet), "utf8");
  fs.writeFileSync(handoffFile, buildAgentHandoff(packet, options), "utf8");
  return {
    schema: "kaizen7.forge_session_write.v1",
    version: 1,
    status: "written",
    ...
    next_action: "review_forge_session",
  };
}
...
function buildUniversalCapabilityForgePacket(objective = "", options = {}) {
  ...
  const adapterManifest = buildAdapterManifestPlan(providerRadar, capability);
  return {
    ...
    agent_packet: {
      ...
      first_move: inferAgentFirstMove(providerRadar),
