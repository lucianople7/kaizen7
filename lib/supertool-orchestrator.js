...
function buildSupertoolPlan(options = {}) {
  ...
  const frontierBuilder = typeof options.frontier === "function"
    ? options.frontier
    : (input) => buildFrontierOperatorBrief(input);
  ...
  if (detected.intent === "frontier") {
    const frontierPlan = frontierBuilder({ root, writeSignals: options.writeSignals !== false, limit: options.limit || 5 });
    return {
      ...base,
