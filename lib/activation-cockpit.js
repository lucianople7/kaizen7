...
function chooseNextAction(toolchain, goal) {
  if (toolchain.toolchain.some((item) => item.id === "openhands-worker")) {
    const command = toolchain.commands.find((item) => item.includes("k7:openhands")) || `npm.cmd run k7:openhands -- ${JSON.stringify(goal)}`;
    return {
  ...
  if (toolchain.toolchain.some((item) => item.id === "codex-local")) {
    const command = toolchain.commands.find((item) => item.includes("k7:codex")) || `npm.cmd run k7:codex -- ${JSON.stringify(goal)}`;
    return {
...
function buildMetaskillBoot(options = {}) {
  ...
  const context = normalize(options.context);
  ...
  const contextPart = context ? ` --context ${JSON.stringify(context)}` : "";
  ...
  return {
    ...
    commands: unique([
      ...
      `npm.cmd run k7:cockpit --${contextPart}${capabilitySegment} ${JSON.stringify(goal)}`,
    ]),
  };
}
...
function buildActivationCockpit(options = {}) {
  ...
  const metaskillBoot = buildMetaskillBoot({ goal, context, capabilities, route });
