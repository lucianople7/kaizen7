...
function ownerFor(preflight = {}) {
  const objective = normalize(preflight.objective);
  if (/\b(video|creativ|plantilla|flowmatik)\b|subtitul/.test(objective)) return "flowmatik";
  if (preflight.route === "codex_execute") return "codex";
  return "work";
}

function buildLoopTask(preflight = {}) {
...
function runActionReactionLoop(objective = "", options = {}) {
  ...
  const maxIterations = Number.isInteger(options.maxIterations) ? options.maxIterations : 8;
  const tokenBudget = Number.isInteger(options.tokenBudget) ? options.tokenBudget : 1200;
  const states = [];
  let tokensUsed = 0;

  const base = { objective: goal, states, task: null, receipt: null, learning: { promoted: false, verified_uses: 0 } };
  ...
  const preflight = (options.preflight || buildPreflight)(goal, {
    root: options.root,
    now,
    budget: Math.min(600, Math.max(120, options.preflightBudget || 300)),
  ...
  const task = buildLoopTask(preflight);
  base.task = task;
  ...
  base.learning = {
    promoted: uses >= 3,
    verified_uses: uses,
    rule: uses >= 3 ? receipt.reuse_next_time : "Collect three verified matching outcomes before promotion.",
  };
