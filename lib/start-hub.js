...
function scopedObjective({ project = "", context = "", objective = "" } = {}) {
  return [
    `Project: ${project || "unspecified"}`,
    `Context: ${context || "contexto minimo"}`,
    `Objective: ${objective}`,
  ].join(" | ");
}

function scopeActionCommand(command = "", scope = {}) {
  const objective = normalize(scope.objective);
  const scoped = scopedObjective(scope);
  const quotedObjective = JSON.stringify(objective);
  if (objective && command.includes(quotedObjective)) {
    return command.replace(quotedObjective, JSON.stringify(scoped));
  }
  return `${command} ${JSON.stringify(scoped)}`.trim();
}

function scopeFirstAction(action = {}, scope = {}) {
  return {
    ...action,
    command: scopeActionCommand(action.command, scope),
  };
}
...
function buildStartHub(options = {}) {
  ...
  const firstAction = scopeFirstAction(cockpit.nextAction, { project, context, objective });
  const cockpitCommands = cockpit.commands.map((command) => (
    command === cockpit.nextAction.command ? firstAction.command : command
  ));
