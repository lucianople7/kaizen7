...
(async () => {
  try {
    ...
    const capabilityRunCardResponse = await fetch(`http://localhost:${port}/api/k7/capabilities/run-card`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        objective: "crear reel de Mr Kaizen con evidencia",
    ...
    assert.equal(capabilityRunCardResponse.status, 200);
    ...
    const capabilityMutualLearningResponse = await fetch(`http://localhost:${port}/api/k7/capabilities/mutual-learn`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        objective: "crear reel de Mr Kaizen con evidencia",
    ...
    assert.equal(capabilityMutualLearningResponse.status, 200);
