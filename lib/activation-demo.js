...
function buildAiHandoff({ goal, agent, after, launchCard, activationPack, projectFocus }) {
  const objective = compactGoal(goal);
  const outputSchema = {
    result: "string",
    proof: "string",
  ...
  return {
    ...
    constraints: {
      ...
      focusGuard: projectFocus?.guard || { decision: "clarify" },
...
async function buildActivationDemo(options = {}) {
  ...
  const aiHandoff = buildAiHandoff({ goal, agent, after, launchCard, activationPack, projectFocus });
  return {
    version: 1,
