...
function inferGrowthLane(mission = {}) {
  ...
  if (/kaizen7|kernel|capability|mission|brief|receipt|friccion|contexto|verificacion/.test(text)) return "KAIZEN7 Execution Quality";
...
function buildMissionControl(rawMission = {}, options = {}) {
  const mission = {
    ...
    capability: clean(rawMission.capability),
...
function formatMissionControl(control) {
  return [
    ...
    `Capability: ${control.mission_brief.capability}`,
