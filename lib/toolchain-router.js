...
function selectToolchain(goal, capabilities = []) {
  ...
  if (includesAny(text, [/openhands|remote worker|worker remoto|external worker|worker_externo/])) {
    tools.push(tool("openhands-worker", "execution", "delegate bounded coding work and require diff/test evidence"));
  } else if (includesAny(text, [/coding|code|codigo|tests?|run_tests|edit_files|refactor|implementar|implementation|mejorar/])) {
    tools.push(tool("codex-local", "execution", "execute locally with Codex using scoped context and fresh verification"));
...
function commandFor(toolItem, goal, capabilities = []) {
  const capabilityPart = capabilityArgs(capabilities);
  const capabilitySegment = capabilityPart ? ` ${capabilityPart}` : "";
  if (toolItem.id === "openhands-worker") return `npm.cmd run k7:openhands --${capabilitySegment} ${JSON.stringify(goal)}`;
  if (toolItem.id === "codex-local") return `npm.cmd run k7:codex --${capabilitySegment} ${JSON.stringify(goal)}`;
  if (toolItem.id === "adapter-registry") return `npm.cmd run k7:adapt -- --kind mcp${capabilitySegment} ${JSON.stringify(goal)}`;
  if (toolItem.id === "external-intelligence") return `npm.cmd run k7:connect -- --domain research ${JSON.stringify(goal)}`;
  if (toolItem.id === "connector-kernel") return `npm.cmd run k7:connect --${capabilitySegment} ${JSON.stringify(goal)}`;
  if (toolItem.id === "semantic-memory") return `npm.cmd run k7:memory --${capabilitySegment} ${JSON.stringify(goal)}`;
  return "";
}
...
function buildToolchainPlan(options = {}) {
  ...
  const commands = unique(toolchain.map((item) => commandFor(item, goal, capabilities)).filter(Boolean));
